...
pub fn run(runtime: Arc<sim::Runtime>) -> anyhow::Result<bool> {
    ...
    let refresh_snapshot = !std::env::var("SKYBOOK_RUNTIME_TEST_REFRESH").unwrap_or_default().is_empty();
    if refresh_snapshot {
    ...
    for entry in test_dir {
        ...
        let test_name = file_name.strip_suffix(".txt").expect("test should end with .txt").to_string();
        test_names.push(test_name);
    }

    let total_count = test_names.len();
    let passed_count = tokio::runtime::Builder::new_current_thread().enable_all().build().context("failed to create tokio runtime")
        ?.block_on(async {
            run_tests(runtime, test_names, refresh_snapshot).await
...
async fn run_tests(runtime: Arc<sim::Runtime>, test_names: Vec<String>, refresh: bool) -> anyhow::Result<usize> {
    let mut handles = vec![];
...
async fn run_test(runtime: &sim::Runtime, refresh: bool, test_name: &str, 
    test_script: &str, parsed_output: Arc<ParseOutput>) -> anyhow::Result<bool> {
    let mut new_snapshot = String::from("// This has RS extension since that usually gives a minimal syntax highlighting.\n//This is not an actual RS file\n\nx!{ SKYBOOK RUNTIME SNAPSHOT V1\n\n");
    ...
    let output = run.run_parsed(Arc::clone(&parsed_output), runtime).await.unwrap();

    // also write diagnostics into output
    for error in output.errors {
        let prefix = if error.is_warning { "warning: " } else {  "error: " };
        new_snapshot += &format!("{}: {}\n", prefix, error.error);
    ...
    let old_snapshot_content = std::fs::read_to_string(snapshot_file_path)
