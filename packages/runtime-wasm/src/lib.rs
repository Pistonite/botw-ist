...
use skybook_runtime::{
    erc, error::MaybeAborted, iv, CustomImageInitParams, ResultInterop, RunHandle, RunOutput,
    Runtime, RuntimeInitError, RuntimeInitOutput,
};
...
pub fn init_runtime(
    ...
    let runtime = match Runtime::new_init(custom_image) {
        Ok(runtime) => runtime,
        Err(e) => {
    ...
    let game_version = match runtime.env.game_ver {
        GameVer::X150 => "1.5",
        GameVer::X160 => "1.6",
    };
