...
var createEditorMain = (options) => {
  ...
  if (options.typescript) {
    ...
    lines.push(`const W = (await import('monaco-editor/esm/vs/language/typescript/ts.worker.js?worker')).default;`);
    lines.push(`return new W();`);
    lines.push(`}`);
  }
  if (options.json) {
    lines.push(`if (label === 'json' || label === 'jsonc') {`);
    lines.push(`const W = (await import('monaco-editor/esm/vs/language/json/json.worker.js?worker')).default;`);
    lines.push(`return new W();`);
    lines.push(`}`);
  }
  if (options.css) {
    lines.push(`if (label === 'css' || label === 'scss' || label === 'sass' || label === 'less') {`);
    lines.push(`const W = (await import('monaco-editor/esm/vs/language/css/css.worker.js?worker')).default;`);
    lines.push(`return new W();`);
    lines.push(`}`);
  }
  if (options.html) {
    lines.push(`if (label === 'html' || label === 'htm') {`);
    lines.push(`const W = (await import('monaco-editor/esm/vs/language/html/html.worker.js?worker')).default;`);
    lines.push(`return new W();`);
    lines.push(`}`);
  }
  lines.push(`const W = (await import('monaco-editor/esm/vs/editor/editor.worker.js?worker')).default;`);
  lines.push("return new W();");
  lines.push("}};");
...
var updateRollupOutputConfig = (output) => {
  ...
  const assetFileNames = (info) => {
    ...
    for (let i = 0;i < info.originalFileNames.length; i++) {
      if (info.originalFileNames[i].match(/esm[/\\]vs[/\\]language/) || info.originalFileNames[i].match(/esm[/\\]vs[/\\]editor/) || info.originalFileNames[i].match(/esm[/\\]vs[/\\]base/)) {
        return "assets/intwc/[name]-[hash][extname]";
      }
