use blueflame::{game::{PouchItemType, PouchItemUse}, processor::CrashReport};
...
fn fmt_iv_pouch(pouch: &Result<iv::PouchList, sim::view::Error>, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let pouch = match pouch {
        Err(e) => {
            writeln!(f, "  pouch_error: ({})", e.to_string())?;
            return Ok(())
        }
        Ok(x) => x
    };
