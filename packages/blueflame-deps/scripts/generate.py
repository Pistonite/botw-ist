...
def generate_flag_function(file: str, lines: list[str], max_lengths):
    ...
    with open(os.path.join(DATA_DIR, "output", "GameData", file), "r", encoding="utf-8") as f:
...
def gen_bool_flag_pack():
    ...
    with open(os.path.join(DATA_DIR, "output", "GameData", "Bool.yaml"), "r", encoding="utf-8") as f:
        entries = yaml.safe_load(f)
    bytes = bytearray()
...
def gen_s32_flag_pack():
    ...
    with open(os.path.join(DATA_DIR, "output", "GameData", "S32.yaml"), "r", encoding="utf-8") as f:
        entries = yaml.safe_load(f)
    bytes = bytearray()
    for entry in entries:
        hash = entry["hash"]
