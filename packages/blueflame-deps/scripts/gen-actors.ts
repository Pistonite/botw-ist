...
const canUseActors = new Set<string>();
...
for (const actor in ActorDataMap) {
    ...
    const canUse = getActorParam(actor, "canUse");
    if (canUse) {
        canUseActors.add(actor);
    }
}
...
console.log(canUseActors.size, "eatable actors found");
...
lines.push("#[rustfmt::skip]");
lines.push("pub static EATABLE_ACTORS_SORTED: &[&str] = &[");
const canUseActorsSorted = [...canUseActors];
canUseActorsSorted.sort();
for (const actor of canUseActorsSorted) {
    lines.push(`    "${actor}",`);
}
lines.push("];")
