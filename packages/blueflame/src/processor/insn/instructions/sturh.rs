...
impl Core<'_, '_, '_> {
    pub fn sturh(
        ...
        self.sturh_core(xt, memory_to_read as u64)
    }
    ...
    pub fn sturh_post_idx(
        ...
        self.sturh_core(xt, xn_sp_val as u64)?;
        let new_reg_val = xn_sp_val + imm_val;
        self.cpu.write_gen_reg(&xn_sp, new_reg_val)?;
        Result::Ok(())
    }

    fn sturh_core(&mut self, xt: RegisterType, address: u64) -> Result<(), Error> {
...
pub fn simple_sturh_test() -> anyhow::Result<()> {
    ...
    core.handle_string_command(&String::from("sturh x0, [x1]"))?;
