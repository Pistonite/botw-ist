...
export const makeItemSlotInfo = (actorName: string, rest: Partial<ItemSlotInfo> = {}): ItemSlotInfo => {
    const [itemType, itemUse] = getItemTypeAndUse(actorName);
    const value = isEquipment(itemType) ? getActorParam(actorName, "generalLife") * 100 : 1;
    return {
        actorName,
        itemType,
        itemUse,
...
export const getItemTypeAndUse = (actorName: string): [PouchItemType, ItemUse] => {
    ...
    if (getActorParam(actorName, "isCureItem")) {
        return [isFood ? PouchItemType.Food: PouchItemType.Material, ItemUse.CureItem];
    }
