...
function normal_core_color() {
    return [ "#F0C83D83", "#FE8F0088", "red", "#73FBFD"] as const;
}

function purple_core_color() {
    return [ "#F909D183", "#FE8F0088", "red", "#EE15F4"] as const;
}

function main(
    color_fn: () => readonly [string, string, string, string],
    rings: boolean
) {
    const [color_core1, color_core2, color_core3, color_light] = color_fn();
