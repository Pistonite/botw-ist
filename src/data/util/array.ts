...
export const inPlaceMap = <T>(
    array: T[],
    act: (elem: T, i: number, arr: T[]) => T,
): void => {
    for (let i = 0; i < array.length; i++) {
...
export const circularForEachFromIndex = <T>(
    array: T[],
    from: number,
    act: (elem: T, i: number, arr: T[]) => void,
): void => {
...
export const arrayEqual = <A, B extends Equalable<A>>(
    arrayA: A[],
    arrayB: B[],
): boolean => {
