...
export class ItemStackImpl implements ItemStack {
    ...
    public modify(option: Partial<ItemStack>): ItemStack {
        ...
        newStack.equipped =
            "equipped" in option ? !!option.equipped : this.equipped;
        newStack.exData.cookEffect = option.foodEffect ?? this.foodEffect;
        newStack.exData.modifierType =
            option.weaponModifier ??
            option.foodSellPrice ??
            this.weaponModifier;
        newStack.exData.modifierValue =
            option.weaponValue ?? option.foodHpRecover ?? this.weaponValue;
    ...
    public equalsExcept(
        ...
        if (
            ...
            this.life !== other.count
        ) {
            return false;
        }
        if (
            !keys.includes("foodSellPrice") &&
            !keys.includes("weaponModifier") &&
            this.foodSellPrice !== other.foodSellPrice
        ) {
            return false;
        }
        if (
            !keys.includes("foodHpRecover") &&
            !keys.includes("weaponValue") &&
            this.foodHpRecover !== other.foodHpRecover
        ) {
            return false;
        }
        if (
            !keys.includes("foodEffect") &&
            this.foodEffect !== other.foodEffect
        ) {
            return false;
        }
