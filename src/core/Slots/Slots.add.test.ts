import { ItemStack, ItemType, createMaterialStack, createEquipmentStack } from "data/item";
import { Slots } from "./Slots";
import { createArrowMockItem, createEquipmentMockItem, createFoodMockItem, createKeyMockItem, createMaterialMockItem, equalsExceptEquip } from "data/item/TestHelpers";
	...
	describe("sorted", ()=>{
		describe("reloading = true", ()=>{
			...
			it("should add multiple new stacks when same type is present, =999", ()=>{
				...
				expect(added).toBe(1);
				expect(slots.getSlotsRef()).toEqualItemStacks(expected);
				added = slots.add(stackToAdd, true, null);
				expect(added).toBe(1);
			...
			it("should not auto equip weapon/bow/shield", ()=>{
				...
				let added = slots.add(stackToAdd1, true, null);
				expect(added).toBe(1);
				added = slots.add(stackToAdd2, true, null);
				expect(added).toBe(1);
				added = slots.add(stackToAdd3, true, null);
				expect(added).toBe(1);
		...
		describe("reloading = false", ()=>{
			...
			it("should auto equip weapon/bow/shield when none is there", ()=>{
				...
				let added = slots.add(stackToAdd1, false, null);
				expect(added).toBe(1);
				added = slots.add(stackToAdd2, false, null);
				expect(added).toBe(1);
				added = slots.add(stackToAdd3, false, null);
				expect(added).toBe(1);
	...
	describe("unsorted", ()=>{
		describe("reloading = true", ()=>{
			...
			it("should add unrepeatable if not in first tab", ()=>{
				...
				const slots = new Slots(stacks);
				const added = slots.add(existing3, true, null);
				expect(added).toBe(1);
			...
			it("should skip arrow 999 check for [bow, shield, arrow]", ()=>{
				...
				const slots = new Slots(stacks);
				const added = slots.add(toAdd, true, null);
				expect(added).toBe(1);
			...
			it("should skip arrow 999 check for [bow, weapon, arrow] if mCount = 0", ()=>{
				...
				const toAdd = createMaterialStack(mockItem3, 999);
				const slots = new Slots(stacks);
				const added = slots.add(toAdd, true, 0);
				expect(added).toBe(1);
			...
			it("should add unrepeatable if mCount = 0", ()=>{
				...
				const slots = new Slots(stacks);
				const added = slots.add(existing1, true, 0);
				expect(added).toBe(1);
