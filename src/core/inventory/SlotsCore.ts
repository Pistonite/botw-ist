...
export class SlotsCore {
    ...
    public getMatchingRefs(
        matchers: ((stack: ItemStack) => boolean)[],
    ): Ref<ItemStack>[][] {
    ...
    public swap(i: number, j: number) {
        if (
            i < 0 ||
            j < 0 ||
            i >= this.internalSlots.length ||
            j >= this.internalSlots.length
        ) {
            return;
        }
    ...
    public sortMaterials() {
        stableSort(this.internalSlots, (a, b) => {
            ...
            if (
                itemA.type === ItemType.Material &&
                itemB.type === ItemType.Material
            ) {
    ...
    public sortItemByTab(mCount: number | null) {
        ...
        stableSort(this.internalSlots, (a, b) => {
            ...
            const itemB = b.get().item;
            if (
                itemA.type === ItemType.Arrow &&
                itemB.type === ItemType.Arrow
            ) {
                return itemA.sortOrder - itemB.sortOrder;
            }
    ...
    public removeAll(types: ItemType[]) {
        inPlaceFilter(
            this.internalSlots,
            (ref) => !types.includes(ref.get().item.type),
        );
    ...
    public findFirstTab(
    ...
    ): [Ref<ItemStack> | undefined, number] {
        ...
        if (type === ItemType.Arrow) {
            ...
            for (
                ;
                foundTabItemIndex < this.internalSlots.length;
                foundTabItemIndex++
            ) {
                const type =
                    this.internalSlots[foundTabItemIndex].get().item.type;
                if (type > ItemType.Arrow) {
                    // arrow not found
                    return [undefined, -1];
                }
