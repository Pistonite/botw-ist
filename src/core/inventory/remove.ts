...
export const remove = (
    ...
    const matchers = [
        ...
        (stack: ItemStack) =>
            stack.equalsExcept(stackToRemove, "count", "equipped"),
    ...
    matchedSlots.forEach((matchedArray) => {
        ...
        circularForEachFromIndex(matchedArray, startSlot, (ref) =>
            slotsToRemoveFrom.push(ref),
        );
    });

    for (
        let j = 0;
        j < slotsToRemoveFrom.length &&
        (countLeft === AmountAll || countLeft > 0);
        j++
    ) {
        ...
        if (specialIsStackable(currentStack.item)) {
            if (countLeft !== AmountAll) {
                ...
                if (currentStack.count > countLeft) {
                    ...
                    ref.set(
                        currentStack.modify({
                            count: currentStack.count - countLeft,
                        }),
                    );
                    countLeft = 0;
                } else {
