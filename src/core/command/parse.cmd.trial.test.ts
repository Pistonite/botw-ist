...
describe("core/command/parse.trial", ()=>{
	...
	it.each([
		...
		expect(`enter ${trial}`).toParseIntoCommand(undefined, new CommandTrial(true, []));
		expect(`exit ${trial}`).toParseIntoCommand(undefined, new CommandTrial(false, []));
		expect(`leave ${trial}`).toParseIntoCommand(undefined, new CommandTrial(false, []));
