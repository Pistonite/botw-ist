...
describe("core/command/parse.initgamedata", () => {
    ...
    it("parses hint when failed", () => {
        expect("initialize gamedata ???").toParseIntoCommand(
            mockSearchItem,
            CmdErr.Guess,
        );
    });
    it("parses empty items", () => {
        expect("initialize gamedata").toParseIntoCommand(
            mockSearchItem,
            new CommandInitGameData([], []),
        );
    });
    it("parses single item", () => {
        expect("initialize gamedata Material A").toParseIntoCommand(
            mockSearchItem,
            new CommandInitGameData(
    ...
    it("parses list of items", () => {
        expect(
            "init gamedata 1 Material A 2 Material B 3 Material C",
        ).toParseIntoCommand(
            mockSearchItem,
            new CommandInitGameData(
                [
    ...
    it("parses list of items with meta", () => {
        expect(
            "init gamedata 1 Material A 2 Material B 1 weapon1 [equip]",
        ).toParseIntoCommand(
            mockSearchItem,
            new CommandInitGameData(
                [
    ...
    it("parses repeated as separate slots", () => {
        expect("init gamedata 2 materialb 2 materialb").toParseIntoCommand(
            mockSearchItem,
            new CommandInitGameData(
                [
                    new ItemStackArg(MockItems.materialb.defaultStack, 2),
