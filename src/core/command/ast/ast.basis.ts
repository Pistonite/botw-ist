...
export const isInteger = <T extends {type: string}>(node: T | ASTInteger | null): node is ASTInteger => Boolean(node && node.type === "ASTInteger");
...
export const isIdentifier = <T extends {type: string}>(node: T | ASTIdentifier | null): node is ASTIdentifier => Boolean(node && node.type === "ASTIdentifier");
