...
export const isEpsilon = <T>(node: T | ASTEpsilon): node is ASTEpsilon =>
    node === null;
...
export const isSuperCommandForCommand = <T extends { type: string }>(
    node: T | ASTSuperCommandForCommand | null,
): node is ASTSuperCommandForCommand =>
    Boolean(node && node.type === "ASTSuperCommandForCommand");
...
const parseSuperCommandForCommand: ParseFunction<ASTSuperCommandForCommand> = (
    tokens,
) => {
    ...
    const literal0 = [
        rangeTokens[0].start,
        rangeTokens[rangeTokens.length - 1].end,
    ] as const;
...
export type ASTSuperCommand =
    | ASTSuperCommandAddSlot
    | ASTSuperCommandRemoveSlot
    | ASTSuperCommandSwap
    | ASTSuperCommandSortMaterial
    | ASTSuperCommandForCommand;
// (derivation) CommandInitGameData => LiteralInitialize <gamedata> ZeroOrMoreItems
export const isCommandInitGameData = <T extends { type: string }>(
    node: T | ASTCommandInitGameData | null,
): node is ASTCommandInitGameData =>
    Boolean(node && node.type === "ASTCommandInitGameData");
...
const parseCommandInitGameData: ParseFunction<ASTCommandInitGameData> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isCommandInitialize = <T extends { type: string }>(
    node: T | ASTCommandInitialize | null,
): node is ASTCommandInitialize =>
    Boolean(node && node.type === "ASTCommandInitialize");
export type ASTCommandInitialize = {
    readonly type: "ASTCommandInitialize";
    readonly mLiteralInitialize0: ASTLiteralInitialize;
    readonly mZeroOrMoreItems1: ASTZeroOrMoreItems;
};
const parseCommandInitialize: ParseFunction<ASTCommandInitialize> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isCommandCook = <T extends { type: string }>(
    node: T | ASTCommandCook | null,
): node is ASTCommandCook => Boolean(node && node.type === "ASTCommandCook");
...
const parseCommandCook: ParseFunction<ASTCommandCook> = (tokens) => {
    ...
    const mArgumentOneOrMoreItemsAllowAllMaybeFromSlot2 =
        parseArgumentOneOrMoreItemsAllowAllMaybeFromSlot(tokens);
...
export const isCommandCookCrit = <T extends { type: string }>(
    node: T | ASTCommandCookCrit | null,
): node is ASTCommandCookCrit =>
    Boolean(node && node.type === "ASTCommandCookCrit");
...
const parseCommandCookCrit: ParseFunction<ASTCommandCookCrit> = (tokens) => {
    ...
    const mArgumentOneOrMoreItemsAllowAllMaybeFromSlot4 =
        parseArgumentOneOrMoreItemsAllowAllMaybeFromSlot(tokens);
...
export const isCommandAdd = <T extends { type: string }>(
    node: T | ASTCommandAdd | null,
): node is ASTCommandAdd => Boolean(node && node.type === "ASTCommandAdd");
...
export const isCommandPickUp = <T extends { type: string }>(
    node: T | ASTCommandPickUp | null,
): node is ASTCommandPickUp =>
    Boolean(node && node.type === "ASTCommandPickUp");
...
export const isCommandRemoveAll = <T extends { type: string }>(
    node: T | ASTCommandRemoveAll | null,
): node is ASTCommandRemoveAll =>
    Boolean(node && node.type === "ASTCommandRemoveAll");
...
export const isCommandRemove = <T extends { type: string }>(
    node: T | ASTCommandRemove | null,
): node is ASTCommandRemove =>
    Boolean(node && node.type === "ASTCommandRemove");
...
const parseCommandRemove: ParseFunction<ASTCommandRemove> = (tokens) => {
    ...
    const mArgumentOneOrMoreItemsAllowAllMaybeFromSlot1 =
        parseArgumentOneOrMoreItemsAllowAllMaybeFromSlot(tokens);
...
export const isCommandDrop = <T extends { type: string }>(
    node: T | ASTCommandDrop | null,
): node is ASTCommandDrop => Boolean(node && node.type === "ASTCommandDrop");
...
export const isCommandEat = <T extends { type: string }>(
    node: T | ASTCommandEat | null,
): node is ASTCommandEat => Boolean(node && node.type === "ASTCommandEat");
...
export const isCommandDnp = <T extends { type: string }>(
    node: T | ASTCommandDnp | null,
): node is ASTCommandDnp => Boolean(node && node.type === "ASTCommandDnp");
...
export const isCommandEquip = <T extends { type: string }>(
    node: T | ASTCommandEquip | null,
): node is ASTCommandEquip => Boolean(node && node.type === "ASTCommandEquip");
...
const parseCommandEquip: ParseFunction<ASTCommandEquip> = (tokens) => {
    ...
    const mArgumentSingleItemMaybeInSlot1 =
        parseArgumentSingleItemMaybeInSlot(tokens);
...
export const isCommandUnequipAll = <T extends { type: string }>(
    node: T | ASTCommandUnequipAll | null,
): node is ASTCommandUnequipAll =>
    Boolean(node && node.type === "ASTCommandUnequipAll");
...
const parseCommandUnequipAll: ParseFunction<ASTCommandUnequipAll> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isCommandUnequip = <T extends { type: string }>(
    node: T | ASTCommandUnequip | null,
): node is ASTCommandUnequip =>
    Boolean(node && node.type === "ASTCommandUnequip");
...
export const isCommandShoot = <T extends { type: string }>(
    node: T | ASTCommandShoot | null,
): node is ASTCommandShoot => Boolean(node && node.type === "ASTCommandShoot");
...
export const isCommandEnterTrial = <T extends { type: string }>(
    node: T | ASTCommandEnterTrial | null,
): node is ASTCommandEnterTrial =>
    Boolean(node && node.type === "ASTCommandEnterTrial");
export type ASTCommandEnterTrial = {
    readonly type: "ASTCommandEnterTrial";
    readonly literal0: readonly [number, number];
    readonly mLiteralTrial1: ASTLiteralTrial;
};
const parseCommandEnterTrial: ParseFunction<ASTCommandEnterTrial> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isCommandExitTrial = <T extends { type: string }>(
    node: T | ASTCommandExitTrial | null,
): node is ASTCommandExitTrial =>
    Boolean(node && node.type === "ASTCommandExitTrial");
...
export const isCommandWriteMetadata = <T extends { type: string }>(
    node: T | ASTCommandWriteMetadata | null,
): node is ASTCommandWriteMetadata =>
    Boolean(node && node.type === "ASTCommandWriteMetadata");
...
const parseCommandWriteMetadata: ParseFunction<ASTCommandWriteMetadata> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentSingleItemMaybeInSlot3 =
        parseArgumentSingleItemMaybeInSlot(tokens);
...
export const isCommandSave = <T extends { type: string }>(
    node: T | ASTCommandSave | null,
): node is ASTCommandSave => Boolean(node && node.type === "ASTCommandSave");
...
export const isCommandReload = <T extends { type: string }>(
    node: T | ASTCommandReload | null,
): node is ASTCommandReload =>
    Boolean(node && node.type === "ASTCommandReload");
...
export const isCommandBreakSlots = <T extends { type: string }>(
    node: T | ASTCommandBreakSlots | null,
): node is ASTCommandBreakSlots =>
    Boolean(node && node.type === "ASTCommandBreakSlots");
...
const parseCommandBreakSlots: ParseFunction<ASTCommandBreakSlots> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mMaybeArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot3 =
        parseMaybeArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot(tokens);
    if (
...
export const isCommandCloseGame = <T extends { type: string }>(
    node: T | ASTCommandCloseGame | null,
): node is ASTCommandCloseGame =>
    Boolean(node && node.type === "ASTCommandCloseGame");
...
export const isCommandSyncGameData = <T extends { type: string }>(
    node: T | ASTCommandSyncGameData | null,
): node is ASTCommandSyncGameData =>
    Boolean(node && node.type === "ASTCommandSyncGameData");
export type ASTCommandSyncGameData = {
    readonly type: "ASTCommandSyncGameData";
    readonly literal0: readonly [number, number];
    readonly literal1: readonly [number, number];
};
const parseCommandSyncGameData: ParseFunction<ASTCommandSyncGameData> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isCommandHas = <T extends { type: string }>(
    node: T | ASTCommandHas | null,
): node is ASTCommandHas => Boolean(node && node.type === "ASTCommandHas");
...
export const isSuperCommandAddSlot = <T extends { type: string }>(
    node: T | ASTSuperCommandAddSlot | null,
): node is ASTSuperCommandAddSlot =>
    Boolean(node && node.type === "ASTSuperCommandAddSlot");
...
const parseSuperCommandAddSlot: ParseFunction<ASTSuperCommandAddSlot> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentOneOrMoreItemsMaybeFromSlot3 =
        parseArgumentOneOrMoreItemsMaybeFromSlot(tokens);
...
export const isSuperCommandRemoveSlot = <T extends { type: string }>(
    node: T | ASTSuperCommandRemoveSlot | null,
): node is ASTSuperCommandRemoveSlot =>
    Boolean(node && node.type === "ASTSuperCommandRemoveSlot");
...
const parseSuperCommandRemoveSlot: ParseFunction<ASTSuperCommandRemoveSlot> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isSuperCommandSwap = <T extends { type: string }>(
    node: T | ASTSuperCommandSwap | null,
): node is ASTSuperCommandSwap =>
    Boolean(node && node.type === "ASTSuperCommandSwap");
...
export const isSuperCommandSortMaterial = <T extends { type: string }>(
    node: T | ASTSuperCommandSortMaterial | null,
): node is ASTSuperCommandSortMaterial =>
    Boolean(node && node.type === "ASTSuperCommandSortMaterial");
...
const parseSuperCommandSortMaterial: ParseFunction<
    ASTSuperCommandSortMaterial
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isLiteralInitialize = <T extends { type: string }>(
    node: T | ASTLiteralInitialize | null,
): node is ASTLiteralInitialize =>
    Boolean(node && node.type === "ASTLiteralInitialize");
export type ASTLiteralInitialize = {
    type: "ASTLiteralInitialize";
    range: [number, number];
};
const parseLiteralInitialize: ParseFunction<ASTLiteralInitialize> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isLiteralAdd = <T extends { type: string }>(
    node: T | ASTLiteralAdd | null,
): node is ASTLiteralAdd => Boolean(node && node.type === "ASTLiteralAdd");
...
export const isLiteralPickUp = <T extends { type: string }>(
    node: T | ASTLiteralPickUp | null,
): node is ASTLiteralPickUp =>
    Boolean(node && node.type === "ASTLiteralPickUp");
...
const parseLiteralPickUp: ParseFunction<ASTLiteralPickUp> = (tokens) => {
    ...
    rangeTokens = [];
    if (
        tokens.consume(rangeTokens) === "pick" &&
        tokens.consume(rangeTokens) === "up"
    ) {
        tokens.pop();
        return {
            type: "ASTLiteralPickUp",
...
const parseLiteralRemoveOrDrop: ParseFunction<ASTLiteralRemoveOrDrop> = (
    tokens,
) => {
...
export const isLiteralDrop = <T extends { type: string }>(
    node: T | ASTLiteralDrop | null,
): node is ASTLiteralDrop => Boolean(node && node.type === "ASTLiteralDrop");
...
export const isLiteralRemove = <T extends { type: string }>(
    node: T | ASTLiteralRemove | null,
): node is ASTLiteralRemove =>
    Boolean(node && node.type === "ASTLiteralRemove");
...
export const isLiteralDnp = <T extends { type: string }>(
    node: T | ASTLiteralDnp | null,
): node is ASTLiteralDnp => Boolean(node && node.type === "ASTLiteralDnp");
...
export const isLiteralLeave = <T extends { type: string }>(
    node: T | ASTLiteralLeave | null,
): node is ASTLiteralLeave => Boolean(node && node.type === "ASTLiteralLeave");
...
export const isLiteralClose = <T extends { type: string }>(
    node: T | ASTLiteralClose | null,
): node is ASTLiteralClose => Boolean(node && node.type === "ASTLiteralClose");
...
export const isLiteralTrial = <T extends { type: string }>(
    node: T | ASTLiteralTrial | null,
): node is ASTLiteralTrial => Boolean(node && node.type === "ASTLiteralTrial");
...
export const isLiteralMaybeAllItemType = <T extends { type: string }>(
    node: T | ASTLiteralMaybeAllItemType | null,
): node is ASTLiteralMaybeAllItemType =>
    Boolean(node && node.type === "ASTLiteralMaybeAllItemType");
...
const parseLiteralMaybeAllItemType: ParseFunction<
    ASTLiteralMaybeAllItemType
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
...
export type ASTLiteralItemType =
    | ASTLiteralWeapon
    | ASTLiteralBow
    | ASTLiteralArrow
    | ASTLiteralShield
...
export const isLiteralWeapon = <T extends { type: string }>(
    node: T | ASTLiteralWeapon | null,
): node is ASTLiteralWeapon =>
    Boolean(node && node.type === "ASTLiteralWeapon");
...
export const isLiteralBow = <T extends { type: string }>(
    node: T | ASTLiteralBow | null,
): node is ASTLiteralBow => Boolean(node && node.type === "ASTLiteralBow");
...
export const isLiteralArrow = <T extends { type: string }>(
    node: T | ASTLiteralArrow | null,
): node is ASTLiteralArrow => Boolean(node && node.type === "ASTLiteralArrow");
...
export const isLiteralShield = <T extends { type: string }>(
    node: T | ASTLiteralShield | null,
): node is ASTLiteralShield =>
    Boolean(node && node.type === "ASTLiteralShield");
...
export const isLiteralArmor = <T extends { type: string }>(
    node: T | ASTLiteralArmor | null,
): node is ASTLiteralArmor => Boolean(node && node.type === "ASTLiteralArmor");
...
export const isLiteralMaterial = <T extends { type: string }>(
    node: T | ASTLiteralMaterial | null,
): node is ASTLiteralMaterial =>
    Boolean(node && node.type === "ASTLiteralMaterial");
...
export const isLiteralFood = <T extends { type: string }>(
    node: T | ASTLiteralFood | null,
): node is ASTLiteralFood => Boolean(node && node.type === "ASTLiteralFood");
...
export const isLiteralKeyItem = <T extends { type: string }>(
    node: T | ASTLiteralKeyItem | null,
): node is ASTLiteralKeyItem =>
    Boolean(node && node.type === "ASTLiteralKeyItem");
...
export const isLiteralItem = <T extends { type: string }>(
    node: T | ASTLiteralItem | null,
): node is ASTLiteralItem => Boolean(node && node.type === "ASTLiteralItem");
...
export const isLiteralSlot = <T extends { type: string }>(
    node: T | ASTLiteralSlot | null,
): node is ASTLiteralSlot => Boolean(node && node.type === "ASTLiteralSlot");
...
export const isLiteralAll = <T extends { type: string }>(
    node: T | ASTLiteralAll | null,
): node is ASTLiteralAll => Boolean(node && node.type === "ASTLiteralAll");
...
export const isLiteralNot = <T extends { type: string }>(
    node: T | ASTLiteralNot | null,
): node is ASTLiteralNot => Boolean(node && node.type === "ASTLiteralNot");
...
const parseMaybeArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot: ParseFunction<
...
export type ASTMaybeArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot =
    | ASTArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot
    | ASTEpsilon;
// (derivation) ArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot => <with> ArgumentOneOrMoreItemsAllowAllMaybeFromSlot
export const isArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot = <
    T extends { type: string },
>(
    node: T | ASTArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot | null,
): node is ASTArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot =>
    Boolean(
        node &&
            node.type === "ASTArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot",
    );
...
const parseArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot: ParseFunction<
    ASTArgumentWithOneOrMoreItemsAllowAllMaybeFromSlot
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isArgumentSingleItemMaybeInSlot = <T extends { type: string }>(
    node: T | ASTArgumentSingleItemMaybeInSlot | null,
): node is ASTArgumentSingleItemMaybeInSlot =>
    Boolean(node && node.type === "ASTArgumentSingleItemMaybeInSlot");
...
const parseArgumentSingleItemMaybeInSlot: ParseFunction<
...
> = (tokens) => {
    ...
    const mArgumentSingleItemMaybeInSlotAIdentifier1 =
        parseArgumentSingleItemMaybeInSlotAIdentifier(tokens);
...
const parseArgumentSingleItemMaybeInSlotAIdentifier: ParseFunction<
...
export type ASTArgumentSingleItemMaybeInSlotAIdentifier =
    | ASTClauseInSlot
...
export const isArgumentSingleItemMaybeInSlotAIdentifierC2 = <
    T extends { type: string },
>(
    node: T | ASTArgumentSingleItemMaybeInSlotAIdentifierC2 | null,
): node is ASTArgumentSingleItemMaybeInSlotAIdentifierC2 =>
    Boolean(
        node && node.type === "ASTArgumentSingleItemMaybeInSlotAIdentifierC2",
    );
...
const parseArgumentSingleItemMaybeInSlotAIdentifierC2: ParseFunction<
    ASTArgumentSingleItemMaybeInSlotAIdentifierC2
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
...
const parseArgumentOneOrMoreItemsAllowAllMaybeFromSlot: ParseFunction<
...
export type ASTArgumentOneOrMoreItemsAllowAllMaybeFromSlot =
...
export const isArgumentSingleItemAllowAllMaybeFromSlot = <
    T extends { type: string },
>(
    node: T | ASTArgumentSingleItemAllowAllMaybeFromSlot | null,
): node is ASTArgumentSingleItemAllowAllMaybeFromSlot =>
    Boolean(node && node.type === "ASTArgumentSingleItemAllowAllMaybeFromSlot");
...
const parseArgumentSingleItemAllowAllMaybeFromSlot: ParseFunction<
    ASTArgumentSingleItemAllowAllMaybeFromSlot
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentSingleItemAllowAllMaybeFromSlotAIdentifier1 =
        parseArgumentSingleItemAllowAllMaybeFromSlotAIdentifier(tokens);
    if (
        mArgumentSingleItemAllowAllMaybeFromSlotAIdentifier1 === ParseResultFail
    ) {
        tokens.restore();
        tokens.pop();
        return ParseResultFail;
...
const parseArgumentSingleItemAllowAllMaybeFromSlotAIdentifier: ParseFunction<
...
export type ASTArgumentSingleItemAllowAllMaybeFromSlotAIdentifier =
    | ASTClauseFromSlot
    | ASTArgumentSingleItemAllowAllMaybeFromSlot
...
export const isArgumentSingleItemAllowAllMaybeFromSlotAIdentifierC1 = <
    T extends { type: string },
>(
    node: T | ASTArgumentSingleItemAllowAllMaybeFromSlotAIdentifierC1 | null,
): node is ASTArgumentSingleItemAllowAllMaybeFromSlotAIdentifierC1 =>
    Boolean(
        node &&
            node.type ===
                "ASTArgumentSingleItemAllowAllMaybeFromSlotAIdentifierC1",
...
const parseArgumentSingleItemAllowAllMaybeFromSlotAIdentifierC1: ParseFunction<
    ASTArgumentSingleItemAllowAllMaybeFromSlotAIdentifierC1
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isArgumentItemStacksAllowAllMaybeFromSlot = <
    T extends { type: string },
>(
    node: T | ASTArgumentItemStacksAllowAllMaybeFromSlot | null,
): node is ASTArgumentItemStacksAllowAllMaybeFromSlot =>
    Boolean(node && node.type === "ASTArgumentItemStacksAllowAllMaybeFromSlot");
...
const parseArgumentItemStacksAllowAllMaybeFromSlot: ParseFunction<
    ASTArgumentItemStacksAllowAllMaybeFromSlot
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentItemStacksAllowAllMaybeFromSlotAIdentifier2 =
        parseArgumentItemStacksAllowAllMaybeFromSlotAIdentifier(tokens);
    if (
        mArgumentItemStacksAllowAllMaybeFromSlotAIdentifier2 === ParseResultFail
    ) {
        tokens.restore();
        tokens.pop();
        return ParseResultFail;
...
const parseArgumentItemStacksAllowAllMaybeFromSlotAIdentifier: ParseFunction<
...
export type ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifier =
    | ASTClauseFromSlot
    | ASTArgumentItemStacksAllowAllMaybeFromSlot
...
export const isArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC1 = <
    T extends { type: string },
>(
    node: T | ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC1 | null,
): node is ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC1 =>
    Boolean(
        node &&
            node.type ===
                "ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC1",
...
const parseArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC1: ParseFunction<
    ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC1
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentItemStacksAllowAllMaybeFromSlotAMetadata1 =
        parseArgumentItemStacksAllowAllMaybeFromSlotAMetadata(tokens);
    if (
        mArgumentItemStacksAllowAllMaybeFromSlotAMetadata1 === ParseResultFail
    ) {
        tokens.restore();
        tokens.pop();
        return ParseResultFail;
...
export const isArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC2 = <
    T extends { type: string },
>(
    node: T | ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC2 | null,
): node is ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC2 =>
    Boolean(
        node &&
            node.type ===
                "ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC2",
...
const parseArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC2: ParseFunction<
    ASTArgumentItemStacksAllowAllMaybeFromSlotAIdentifierC2
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentItemStacksAllowAllMaybeFromSlotAIdentifier1 =
        parseArgumentItemStacksAllowAllMaybeFromSlotAIdentifier(tokens);
    if (
        mArgumentItemStacksAllowAllMaybeFromSlotAIdentifier1 === ParseResultFail
    ) {
        tokens.restore();
        tokens.pop();
        return ParseResultFail;
...
const parseArgumentItemStacksAllowAllMaybeFromSlotAMetadata: ParseFunction<
...
export type ASTArgumentItemStacksAllowAllMaybeFromSlotAMetadata =
    | ASTClauseFromSlot
    | ASTArgumentItemStacksAllowAllMaybeFromSlot
    | ASTEpsilon;
// (derivation union) ArgumentOneOrMoreItemsMaybeFromSlot => ArgumentSingleItemMaybeFromSlot | ArgumentItemStacksMaybeFromSlot
const parseArgumentOneOrMoreItemsMaybeFromSlot: ParseFunction<
...
export type ASTArgumentOneOrMoreItemsMaybeFromSlot =
...
export const isArgumentSingleItemMaybeFromSlot = <T extends { type: string }>(
    node: T | ASTArgumentSingleItemMaybeFromSlot | null,
): node is ASTArgumentSingleItemMaybeFromSlot =>
    Boolean(node && node.type === "ASTArgumentSingleItemMaybeFromSlot");
...
const parseArgumentSingleItemMaybeFromSlot: ParseFunction<
    ASTArgumentSingleItemMaybeFromSlot
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentSingleItemMaybeFromSlotAIdentifier1 =
        parseArgumentSingleItemMaybeFromSlotAIdentifier(tokens);
...
const parseArgumentSingleItemMaybeFromSlotAIdentifier: ParseFunction<
...
export type ASTArgumentSingleItemMaybeFromSlotAIdentifier =
    | ASTClauseFromSlot
...
export const isArgumentSingleItemMaybeFromSlotAIdentifierC1 = <
    T extends { type: string },
>(
    node: T | ASTArgumentSingleItemMaybeFromSlotAIdentifierC1 | null,
): node is ASTArgumentSingleItemMaybeFromSlotAIdentifierC1 =>
    Boolean(
        node && node.type === "ASTArgumentSingleItemMaybeFromSlotAIdentifierC1",
    );
...
const parseArgumentSingleItemMaybeFromSlotAIdentifierC1: ParseFunction<
    ASTArgumentSingleItemMaybeFromSlotAIdentifierC1
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isArgumentItemStacksMaybeFromSlot = <T extends { type: string }>(
    node: T | ASTArgumentItemStacksMaybeFromSlot | null,
): node is ASTArgumentItemStacksMaybeFromSlot =>
    Boolean(node && node.type === "ASTArgumentItemStacksMaybeFromSlot");
...
const parseArgumentItemStacksMaybeFromSlot: ParseFunction<
    ASTArgumentItemStacksMaybeFromSlot
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentItemStacksMaybeFromSlotAIdentifier2 =
        parseArgumentItemStacksMaybeFromSlotAIdentifier(tokens);
...
const parseArgumentItemStacksMaybeFromSlotAIdentifier: ParseFunction<
...
export type ASTArgumentItemStacksMaybeFromSlotAIdentifier =
    | ASTClauseFromSlot
...
export const isArgumentItemStacksMaybeFromSlotAIdentifierC1 = <
    T extends { type: string },
>(
    node: T | ASTArgumentItemStacksMaybeFromSlotAIdentifierC1 | null,
): node is ASTArgumentItemStacksMaybeFromSlotAIdentifierC1 =>
    Boolean(
        node && node.type === "ASTArgumentItemStacksMaybeFromSlotAIdentifierC1",
    );
...
const parseArgumentItemStacksMaybeFromSlotAIdentifierC1: ParseFunction<
    ASTArgumentItemStacksMaybeFromSlotAIdentifierC1
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentItemStacksMaybeFromSlotAMetadata1 =
        parseArgumentItemStacksMaybeFromSlotAMetadata(tokens);
...
export const isArgumentItemStacksMaybeFromSlotAIdentifierC2 = <
    T extends { type: string },
>(
    node: T | ASTArgumentItemStacksMaybeFromSlotAIdentifierC2 | null,
): node is ASTArgumentItemStacksMaybeFromSlotAIdentifierC2 =>
    Boolean(
        node && node.type === "ASTArgumentItemStacksMaybeFromSlotAIdentifierC2",
    );
...
const parseArgumentItemStacksMaybeFromSlotAIdentifierC2: ParseFunction<
    ASTArgumentItemStacksMaybeFromSlotAIdentifierC2
> = (tokens) => {
    let rangeTokens: Token[];
    tokens.push();
    ...
    const mArgumentItemStacksMaybeFromSlotAIdentifier1 =
        parseArgumentItemStacksMaybeFromSlotAIdentifier(tokens);
...
const parseArgumentItemStacksMaybeFromSlotAMetadata: ParseFunction<
...
export type ASTArgumentItemStacksMaybeFromSlotAMetadata =
    | ASTClauseFromSlot
    | ASTArgumentItemStacksMaybeFromSlot
    | ASTEpsilon;
// (derivation union) MaybeClauseFromSlot => ClauseFromSlot | Epsilon
const parseMaybeClauseFromSlot: ParseFunction<ASTMaybeClauseFromSlot> = (
    tokens,
) => {
...
export const isClauseFromSlot = <T extends { type: string }>(
    node: T | ASTClauseFromSlot | null,
): node is ASTClauseFromSlot =>
    Boolean(node && node.type === "ASTClauseFromSlot");
...
export const isClauseInSlot = <T extends { type: string }>(
    node: T | ASTClauseInSlot | null,
): node is ASTClauseInSlot => Boolean(node && node.type === "ASTClauseInSlot");
...
const parseMaybeClauseSaveTarget: ParseFunction<ASTMaybeClauseSaveTarget> = (
    tokens,
) => {
...
export const isClauseSaveTarget = <T extends { type: string }>(
    node: T | ASTClauseSaveTarget | null,
): node is ASTClauseSaveTarget =>
    Boolean(node && node.type === "ASTClauseSaveTarget");
...
export const isOneOrMoreItemStacks = <T extends { type: string }>(
    node: T | ASTOneOrMoreItemStacks | null,
): node is ASTOneOrMoreItemStacks =>
    Boolean(node && node.type === "ASTOneOrMoreItemStacks");
...
const parseOneOrMoreItemStacks: ParseFunction<ASTOneOrMoreItemStacks> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isItemStack = <T extends { type: string }>(
    node: T | ASTItemStack | null,
): node is ASTItemStack => Boolean(node && node.type === "ASTItemStack");
...
export const isSingleItem = <T extends { type: string }>(
    node: T | ASTSingleItem | null,
): node is ASTSingleItem => Boolean(node && node.type === "ASTSingleItem");
...
export const isOneOrMoreIdentifiers = <T extends { type: string }>(
    node: T | ASTOneOrMoreIdentifiers | null,
): node is ASTOneOrMoreIdentifiers =>
    Boolean(node && node.type === "ASTOneOrMoreIdentifiers");
export type ASTOneOrMoreIdentifiers = {
    readonly type: "ASTOneOrMoreIdentifiers";
    readonly mIdentifier0: ASTIdentifier;
    readonly mIdentifierPrime1: ASTIdentifierPrime;
};
const parseOneOrMoreIdentifiers: ParseFunction<ASTOneOrMoreIdentifiers> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
...
export const isMetadata = <T extends { type: string }>(
    node: T | ASTMetadata | null,
): node is ASTMetadata => Boolean(node && node.type === "ASTMetadata");
...
export const isKeyValuePair = <T extends { type: string }>(
    node: T | ASTKeyValuePair | null,
): node is ASTKeyValuePair => Boolean(node && node.type === "ASTKeyValuePair");
...
export const isSpecifiedValue = <T extends { type: string }>(
    node: T | ASTSpecifiedValue | null,
): node is ASTSpecifiedValue =>
    Boolean(node && node.type === "ASTSpecifiedValue");
...
export const isValueSpecifier = <T extends { type: string }>(
    node: T | ASTValueSpecifier | null,
): node is ASTValueSpecifier =>
    Boolean(node && node.type === "ASTValueSpecifier");
...
const parseKeyValuePairPrime: ParseFunction<ASTKeyValuePairPrime> = (
    tokens,
) => {
...
export const isKeyValuePairPrimeC1 = <T extends { type: string }>(
    node: T | ASTKeyValuePairPrimeC1 | null,
): node is ASTKeyValuePairPrimeC1 =>
    Boolean(node && node.type === "ASTKeyValuePairPrimeC1");
...
const parseKeyValuePairPrimeC1: ParseFunction<ASTKeyValuePairPrimeC1> = (
    tokens,
) => {
    let rangeTokens: Token[];
    tokens.push();
