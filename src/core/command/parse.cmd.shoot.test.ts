...
describe("core/command/parse.shoot", ()=>{
	...
	it.each([
		...
		expect(`shoot ${input} arrows`).toParseIntoCommand(undefined, new CommandShootArrow(output, []));
		expect(`shoot ${input} arrow`).toParseIntoCommand(undefined, new CommandShootArrow(output, []));
	});
});
