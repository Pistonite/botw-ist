    ...
    const displaySlots = useMemo(() => {
        if (!searchString) {
            return Object.values(allItems).map((item) =>
                new SlotDisplayForItemStack(item.defaultStack).init(
                    false,
                    isIconAnimated,
                ),
            );
    ...
    return (
        <Page title="Item Reference">
            <Category title="Item Slot">
                ...
                />
                <Description>
                    The value at the bottom left is either the durability of the
                    equipment, or the stack size of the item.
                </Description>
                <Label>Offset i.e. broken slots:</Label>
                <ItemSlot
                    slot={{
                        image: "assets/img/Weapon/MasterSword.png",
                        count: undefined,
                ...
                />
                <Description>
                    If the slot has a dark red background, it is referred to as
                    a<span className="Highlight"> "broken slot"</span>. This
                    slot won't be removed on reload.
                </Description>
                ...
                <Description className="Primary">
                    The blue background on the slot indicates that it will
                    appear as equipped in the inventory.
                </Description>
                <Description>
                    Note that multiple slots could appear equipped in the
                    inventory, and it does not necessarily mean it is equipped
                    in the overword.
                </Description>
            </Category>
            <Category title="Explorer">
                <Description className="Primary">
                    Type item name (or part of item name) below to filter the
                    items.
                </Description>
                <Description>
                    The command system searches the items in the same way. After
                    the items are filtered down to the list below based on the
                    search string, it picks the one marked{" "}
