...
/*import-validation-exempt*/ import {
    CrashScreen,
    LoadingScreen,
} from "ui/surfaces";
...
const ItemContext = React.createContext<ItemContextFunctions>(
    {} as ItemContextFunctions,
);
...
export const ItemProvider: React.FC<PropsWithChildren> = ({ children }) => {
    ...
    const [itemSearchMap, setItemSearchMap] = useState<ItemSearchMap | null>(
        null,
    );
    ...
    const [getItem, searchItem, getAllItems] = useMemo(() => {
        ...
        if (itemIdMap && itemSearchMap) {
            ...
            const searchItem = (
                ...
                const [result, otherResults] = searchItemMemoized(
                    word,
                    itemIdMap,
                    itemSearchMap,
                    MemoizedSearchResults,
...
export const loadItemData = (
    ...
    registerItemCategoryByName(
        itemData,
        "armor",
        ItemType.ArmorUpper, // will be updated in registerItem
        idMap,
        searchMap,
    );
    registerItemCategoryByName(
...
const registerItemCategoryByName = (
    itemData: ItemData,
    category: keyof ItemData,
    type: ItemType,
    outIdMap: ItemIdMap,
    outSearchMap: ItemSearchMap,
) => {
...
const registerItem = (
    ...
    const animatedImage = option.animated
        ? getImageUrl(id, type, true)
        : undefined;
    const stackable = option.repeatable && option.stackable;

    // default stack
    let defaultStackFactory: ((item: Item) => ItemStack) | undefined =
        undefined;
    if (
...
const searchItemInMap = (
    ...
    for (let i = 0; i < tries.length; i++) {
        ...
        if (name.endsWith(find)) {
            const newName =
                name.substring(0, name.length - find.length) + replace;
            const attemptResult = searchItemInMapCore(
                newName,
                idMap,
                searchMap,
            );
    ...
    results.forEach((resultId) => {
        scoreMap[resultId] = longestCommonSubstringLength(
            resultId.toLowerCase(),
            name.toLowerCase(),
        );
    });
    const typeOrder = [ItemType.Arrow, ItemType.Material];
    results.sort((a, b) => {
        const itemA = idMap[a];
        const itemB = idMap[b];
        // Prioritize:
