...
export class VisibleInventory implements DisplayableInventory {
    ...
    public getDisplayedSlots(isIconAnimated: boolean): SlotDisplay[] {
        ...
        const result = this.slots
            .getView()
            .map((stack, i) =>
                new SlotDisplayForItemStack(stack).init(
                    i >= mCount,
                    isIconAnimated,
                ),
            );
    ...
    public addWhenReload(
        stack: ItemStack,
        cookDataSource: ItemStack,
        lastAdded: Ref<ItemStack> | undefined,
        flags: GameFlags,
    ): Ref<ItemStack> | undefined {
        const newlyAdded = this.slots.add(
            stack,
            true,
            this.getMCount(),
            flags,
    ...
    public updateEquipmentDurability(gameData: GameData) {
        ...
        if (firstEquippedWeaponSlot >= 0 && lastEquippedWeapon) {
            gameData.updateLife(
                lastEquippedWeapon.get().count,
                firstEquippedWeaponSlot,
            );
            this.slots.updateLife(
                lastEquippedWeapon.get().count,
                firstEquippedWeaponSlot,
            );
        }
        ...
        if (firstEquippedBowSlot >= 0 && lastEquippedBow) {
            gameData.updateLife(
                lastEquippedBow.get().count,
                firstEquippedBowSlot,
            );
            this.slots.updateLife(
                lastEquippedBow.get().count,
                firstEquippedBowSlot,
        ...
        if (firstEquippedShieldSlot >= 0 && lastEquippedShield) {
            gameData.updateLife(
                lastEquippedShield.get().count,
                firstEquippedShieldSlot,
            );
            this.slots.updateLife(
                lastEquippedShield.get().count,
                firstEquippedShieldSlot,
