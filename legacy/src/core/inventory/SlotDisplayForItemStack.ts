...
export class SlotDisplayForItemStack implements SlotDisplay {
    ...
    public init(isBrokenSlot: boolean, isIconAnimated: boolean): SlotDisplay {
    ...
    get image(): string {
        return this.isIconAnimated
            ? this.stack.item.animatedImage
            : this.stack.item.image;
    }
    ...
    get durability(): string | undefined {
        ...
        return Number.isInteger(durability)
            ? durability + ""
            : durability.toPrecision(4);
    }
    ...
    get modifierImage(): string | undefined {
        ...
        const isYellow =
            (this.stack.weaponModifier & WeaponModifier.Yellow) !==
            WeaponModifier.None;
    ...
    get modifierText(): string {
        ...
        if (selectedModifier) {
            ...
            if (
                (selectedModifier & WeaponModifier.AttackUp) !==
                WeaponModifier.None
            ) {
                return `+${this.stack.weaponValue}`;
            }
            if (
                (selectedModifier & WeaponModifier.GuardUp) !==
                WeaponModifier.None
            ) {
                let value = this.stack.weaponValue;
                if (
                    (this.stack.weaponModifier & WeaponModifier.AttackUp) !==
                    WeaponModifier.None
                ) {
    ...
    get modifierClassName(): string {
        ...
        return clsx(
            "ItemModifierStringValue",
            (this.stack.weaponModifier & WeaponModifier.Yellow) !==
                WeaponModifier.None && "ItemModifierStringValueYellow",
        );
    }
    ...
    public getTooltip(translate: (s: string) => string): [string, string][] {
        ...
        const hasExtraFoodData =
            foodHpRecover > 0 ||
            foodSellPrice > 0 ||
            foodEffect !== CookEffect.None;
        const isFoodModifierInactive = item.stackable && hasExtraFoodData;
        const foodModifierClass = isFoodModifierInactive
            ? "ItemTooltipFoodEffectInactive"
            : "ItemTooltipFoodEffect";
        return [
            ...
            item.bowRapidfire && [
                `Initial Rapidfire x${item.bowRapidfire}`,
                "",
            ],
            item.bowMultishot && [
                `Initial Multishot x${item.bowMultishot}`,
                "",
            ],

            isFood &&
                foodEffect !== CookEffect.None && [
                    `Food Effect: ${CookEffect[foodEffect]}`,
                    foodModifierClass,
                ],
            isFood &&
                foodHpRecover > 0 && [
                    `Recover ${foodHpRecover / 4} Hearts`,
                    foodModifierClass,
                ],
            isFood &&
                foodSellPrice > 0 && [
                    `Sell Price: ${foodSellPrice}`,
                    foodModifierClass,
                ],
            isFood &&
                isFoodModifierInactive && [
                    "Food data is not visible",
                    foodModifierClass,
                ],

            isEquipment &&
                (weaponModifier & WeaponModifier.AttackUp) !== 0 && [
                    `Attack +${weaponValue}`,
                    "ItemTooltipWeaponModifier",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.DurabilityUp) !== 0 && [
                    "Durablity Up",
                    "ItemTooltipWeaponModifier",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.CriticalHit) !== 0 && [
                    "Critical Hit",
                    item.type === ItemType.Weapon
                        ? "ItemTooltipWeaponModifier"
                        : "ItemTooltipWeaponModifierInactive",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.LongThrow) !== 0 && [
                    `Throw Speed ${getFixedPointReductionString(weaponValue)}`,
                    item.type === ItemType.Weapon
                        ? "ItemTooltipWeaponModifier"
                        : "ItemTooltipWeaponModifierInactive",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.MultiShot) !== 0 && [
                    `Multishot x${Math.min(weaponValue, 10)} Max`,
                    isBow
                        ? "ItemTooltipWeaponModifier"
                        : "ItemTooltipWeaponModifierInactive",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.Zoom) !== 0 && [
                    "Zoom",
                    isBow
                        ? "ItemTooltipWeaponModifier"
                        : "ItemTooltipWeaponModifierInactive",
            ...
            isEquipment &&
                (weaponModifier & WeaponModifier.QuickShot) !== 0 && [
                    `Bow Draw Speed ${getFixedPointReductionString(weaponValue)}`,
                    isBow
                        ? "ItemTooltipWeaponModifier"
                        : "ItemTooltipWeaponModifierInactive",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.SurfMaster) !== 0 && [
                    `Surf Friction =${weaponValue}`,
                    item.type === ItemType.Shield
                        ? "ItemTooltipWeaponModifier"
                        : "ItemTooltipWeaponModifierInactive",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.GuardUp) !== 0 && [
                    `Shield Guard +${weaponValue}`,
                    item.type === ItemType.Shield
                        ? "ItemTooltipWeaponModifier"
                        : "ItemTooltipWeaponModifierInactive",
                ],
            isEquipment &&
                (weaponModifier & WeaponModifier.Yellow) !== 0 && [
                    "Yellow Modifier",
                    "ItemTooltipFoodEffect",
                ],
    ...
    private isEquipment(): boolean {
        return [ItemType.Weapon, ItemType.Bow, ItemType.Shield].includes(
            this.stack.item.type,
        );
    }
}
...
const selectModifier = (stack: ItemStack): number | undefined => {
    ...
    for (let i = 0; i < applicableModifiers.length; i++) {
        if (
            (applicableModifiers[i] & stack.weaponModifier) !==
            WeaponModifier.None
        ) {
