...
export class Slots {
    ...
    public equip(item: Item, slot: number) {
        ...
        const [firstTabItem, firstTabIndex] = this.core.findFirstTab(
            item.type,
            true,
        );
        if (firstTabItem) {
            for (
                let i = firstTabIndex;
                i < this.core.length && this.core.get(i).item.tab === item.tab;
                i++
            ) {
    ...
    public updateLife(life: number, slot: number) {
        ...
        const isMaterialOrMeal =
            type === ItemType.Material || type === ItemType.Food;
        ...
        for (let j = 0; j < this.core.length; j++) {
            ...
            if (item === equippedArrow) {
                if (countToShoot === AmountAll) {
                    ...
                    this.core.modifySlot(j, {
                        count: Math.max(0, count - countToShoot),
                    });
                }
