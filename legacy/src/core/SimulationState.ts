...
export class SimulationState {
    ...
    constructor(
        gameData: GameData,
        manualSave: GameData | null,
        namedSaves: { [name: string]: GameData },
        pouch: VisibleInventory,
    ) {
    ...
    public addSlotsDirectly(stacks: ItemStack[], index?: number) {
        if (index !== undefined) {
            stacks.forEach((stack, i) =>
                this.pouch.addDirectly(stack, index + i),
            );
        } else {
    ...
    public save(name?: string) {
        if (this.isOnEventide) {
            this.errorTitles.push("Save failed");
    ...
    public setEventide(onEventide: boolean) {
        if (onEventide) {
            if (this.isOnEventide) {
                ...
                this.errorMessages.push(
                    "You are in another trial. Please exit first.",
                );
            } else {
            ...
            if (!this.isOnEventide) {
                ...
                this.errorMessages.push(
                    "You are not in a trial. Please enter one first.",
                );
            } else {
    ...
    public get errors(): string[] {
        ...
        if (titleSet.size === 1) {
            ...
            returnResult.push(
                "This command gave multiple errors when executing:",
            );
        }
