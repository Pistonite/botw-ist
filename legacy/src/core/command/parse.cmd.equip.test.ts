...
describe("core/command/parse.equip equip", () => {
    ...
    it("parses equip single word", () => {
        expect("equip materialA").toParseIntoCommand(
            mockSearchItem,
            new CommandEquip(MockItems.materiala, 1, []),
        );
    });
    it("parses equip more words", () => {
        expect("equip mate ria lA").toParseIntoCommand(
            mockSearchItem,
            new CommandEquip(MockItems.materiala, 1, []),
        );
    });
    it("parses equip from slot", () => {
        expect("equip mate ria lA in slot 3").toParseIntoCommand(
            mockSearchItem,
            new CommandEquip(MockItems.materiala, 3, []),
        );
    });
});
...
describe("core/command/parse.equip unequip", () => {
    ...
    it("parses unequip single word", () => {
        expect("unequip materialA").toParseIntoCommand(
            mockSearchItem,
            new CommandUnequip(MockItems.materiala, 1, []),
        );
    });
    it("parses unequip more words", () => {
        expect("unequip mate ria lA").toParseIntoCommand(
            mockSearchItem,
            new CommandUnequip(MockItems.materiala, 1, []),
        );
    });
    it("parses unequip from slot", () => {
        expect("unequip mate ria lA in slot 3").toParseIntoCommand(
            mockSearchItem,
            new CommandUnequip(MockItems.materiala, 3, []),
        );
    });
});
...
describe("core/command/parse.equip unequip all", () => {
    it.each([
        ...
        expect(`unequip all ${typeString}`).toParseIntoCommand(
            undefined,
            expected,
        );
        expect(`unequip all ${typeString}s`).toParseIntoCommand(
            undefined,
            expected,
        );
        expect(`unequip ${typeString}`).toParseIntoCommand(undefined, expected);
        expect(`unequip ${typeString}s`).toParseIntoCommand(
            undefined,
            expected,
        );
    });
