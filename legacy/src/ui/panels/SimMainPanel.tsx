...
export const SimMainPanel: React.FC<SimMainPanelProps> = ({
    ...
    if (simulationState.isCrashed()) {
        ...
        content = (
            <Section
                className="HatenoBackground"
                titleText={
                    <div style={{ display: "flex" }}>
                        <span>
                            Visible Inventory (Count=
                            {simulationState.inventoryMCount})
                        </span>
                        {inventoryInfo}
                    </div>
                }
            ...
            >
                {(() => {
                    ...
                    const gameDataSlots =
                        simulationState.displayableGameData.getDisplayedSlots(
                            isIconAnimated,
                        );
                    const inventorySlots =
                        simulationState.displayablePouch.getDisplayedSlots(
                            isIconAnimated,
                        );
                    for (
                        let i = 0;
                        i < gameDataSlots.length && i < inventorySlots.length;
                        i++
                    ) {
                        doubleSlots.push(
                            <DoubleItemSlot
                    ...
                    if (gameDataSlots.length > inventorySlots.length) {
                        for (
                            let i = inventorySlots.length;
                            i < gameDataSlots.length;
                            i++
                        ) {
                            doubleSlots.push(
                                <DoubleItemSlot
                                    key={i + inventorySlots.length}
                                    first={{ slot: gameDataSlots[i] }}
                                />,
                            );
                        }
                        ...
                        for (
                            let i = gameDataSlots.length;
                            i < inventorySlots.length;
                            i++
                        ) {
                            doubleSlots.push(
                                <DoubleItemSlot
                                    key={i + gameDataSlots.length}
                                    second={{ slot: inventorySlots[i] }}
                                />,
                            );
                        }
        ...
        content = (
            <>
                {showGameData && (
                    ...
                    >
                        <ItemList
                            slots={simulationState.displayableGameData.getDisplayedSlots(
                                isIconAnimated,
                            )}
                        />
                    </Section>
                ...
                >
                    <ItemList
                        slots={simulationState.displayablePouch.getDisplayedSlots(
                            isIconAnimated,
                        )}
                    />
                </Section>
        ...
        >
            ...
            >
                ...
                {command.err.length > 0 &&
                    (showHint || command.cmdErr !== CmdErr.Guess) && (
                        <div
                            className={clsx(
                                "TooltipWindow",
                                command.cmdErr === CmdErr.Parse &&
                                    "TooltipWarn",
                                command.cmdErr === CmdErr.Execute &&
                                    "TooltipError",
                            )}
                            style={{
                                position: "absolute",
