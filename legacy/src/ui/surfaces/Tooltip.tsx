...
type SetTooltipFunction = (
    screenX: number,
    screenY: number,
    tooltip: React.ReactNode[],
) => void;
const TooltipContext = React.createContext<SetTooltipFunction>(() => {
    /* empty */
});
export const TooltipHost: React.FC<PropsWithChildren> = ({ children }) => {
    ...
    const setScreenTooltip = useCallback(
        (x: number, y: number, tooltips: React.ReactNode[]) => {
            setTooltipX(x + 10);
            setTooltipY(y + 10);
            setTooltip(tooltips);
        },
    ...
    return (
        <TooltipContext.Provider value={setScreenTooltip}>
            {children}
            {tooltip.length > 0 && (
                <div
                    ref={toolTipDivRef}
                    className="TooltipWindow"
                    style={{
                        position: "absolute",
                        left: tooltipX,
                        top: tooltipY,
