...
export const ParseCode: React.FC<PropsWithChildren> = ({ children }) => {
    ...
    return <ColoredCodeBlocks blocks={[command.codeBlocks]} value={[text]} />;
};

export const ColoredCodeBlocks: React.FC<ColoredCodeBlocksProps> = ({
    blocks,
    value,
}) => {
    return (
...
const ColoredCodeBlock: React.FC<ColoredCodeBlockProps> = ({
    blocks,
    value,
}) => {
    const mappedProps = useMemo(() => {
        ...
        blocks.forEach(({ color, start, end }) => {
            ...
            toAdd.forEach((sub, i) => {
                const addColor = i < toAdd.length - 1 ? "unknown" : color;
...
const ColoredSingleBlock: React.FC<ColoredSingleBlockProps> = ({
    color,
    value,
}) => {
    return <code className={clsx("CodeBlock", Colors[color])}>{value}</code>;
};
