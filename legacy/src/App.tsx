...
export const App: React.FC = () => {
    ...
    const displayIndex =
        _displayIndex < 0 || _displayIndex >= simulationStates.length
            ? 0
            : _displayIndex;
    ...
    useLayoutEffect(() => {
        ...
        if (panel && selected) {
            ...
            const bottom =
                selected.offsetTop + selected.getBoundingClientRect().height;
    ...
    useEffect(() => {
        window.onkeydown = (e) => {
            if (e.code === "ArrowDown") {
                ...
                while (
                    nextCommandIndex < commandData.length &&
                    commands[nextCommandIndex].shouldSkipWithKeyboard
                ) {
                    nextCommandIndex++;
                }
                if (nextCommandIndex >= commandData.length) {
                    setCommandData(
                        produce(commandData, (newData) => {
                            newData.push("");
                        }),
                    );
                    ...
                    setDisplayIndex(
                        Math.min(commandData.length - 1, nextCommandIndex),
                    );
                }
    ...
    useEffect(() => {
        if (contextMenuState.index >= commandData.length) {
            ...
            if (rect.bottom > window.innerHeight) {
                setContextMenuState(
                    produce(contextMenuState, (newState) => {
    ...
    const middleHeight = showSaves
        ? `calc( 100vh - 40px - ${saveHeight}px )`
        : fullMainHeight;
    ...
    return (
        <div className="Calamity">
            ...
            {contextMenuState.index >= 0 &&
                contextMenuState.index < commands.length && (
                    <div
                        style={{
                            position: "absolute",
                            top: 0,
