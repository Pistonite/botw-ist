...
expect.extend({
    toEqualItemStacks: (received, expected, equals?) => {
        if (!Array.isArray(received)) {
            return {
                message: () => `expected ${received} to be an ItemStack array`,
                pass: false,
            };
        }
        ...
        for (let i = 0; i < received.length; i++) {
            ...
            if (equals) {
                ...
                !expectStack.equals ||
                !expectStack.equals(receivedStack)
            ) {
                return {
                    message: () =>
                        `Differ at index ${i}, expected ${JSON.stringify(expectStack, null, 2)}, got ${JSON.stringify(receivedStack, null, 2)}`,
        ...
        return {
            message: () => "ItemStack arrays are the same",
            pass: true,
        };
    },
});
...
const runE2ETest = (name: string, debug: boolean): [string, string] => {
    ...
    if (debug) {
        fs.writeFileSync(
            "src/__tests__/debug.actual.log",
            resultString,
            "utf-8",
        );
        fs.writeFileSync(
            "src/__tests__/debug.expected.log",
            expectedString,
            "utf-8",
        );
...
expect.extend({
    toPassE2ESimulation: (receivedName: string, expectDebug?: boolean) => {
        const [resultState, expectedState] = runE2ETest(
            receivedName,
            !!expectDebug,
        );
    ...
    toMatchItemSearch: (
        ...
        if (result === undefined || expected === undefined) {
            ...
            return {
                message: () =>
                    `Item search match failed. Actual: ${result && result.item.id}`,
                pass: false,
            };
        }
    ...
    toParseIntoCommand: (
        ...
        search: ItemSearchFunction,
        expectedCommand: Command | CmdErr,
    ) => {
        ...
        if (typeof expectedCommand === typeof CmdErr.AST) {
            if (command.cmdErr !== expectedCommand) {
                return {
                    message: () =>
                        `Parsed cmderr is ${command.cmdErr}, expected: ${expectedCommand}`,
                    pass: false,
                };
            }
            return {
